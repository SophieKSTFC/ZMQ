# -*- coding: utf-8 -*-
# Run test_msgpack.py with your Python interpreter of choice to test the
# correctness of u-msgpack-python!
#
#   $ python2 test_msgpack.py
#   $ python3 test_msgpack.py
#   $ pypy test_msgpack.py
#   $ pypy3 test_msgpack.py
#
...
single_test_vectors = [
    ...
    # Fixext 1
    ["fixext 1", msgpack.ExtType(0x05, b"\x80" * 1), b"\xd4\x05" + b"\x80" * 1],
    # Fixext 2
    ["fixext 2", msgpack.ExtType(0x05, b"\x80" * 2), b"\xd5\x05" + b"\x80" * 2],
    # FixExtType 4
    ["fixExtType 4", msgpack.ExtType(0x05, b"\x80" * 4), b"\xd6\x05" + b"\x80" * 4],
    # FixExtType 8
    ["fixExtType 8", msgpack.ExtType(0x05, b"\x80" * 8), b"\xd7\x05" + b"\x80" * 8],
    # FixExtType 16
    ["fixExtType 16", msgpack.ExtType(0x05, b"\x80" * 16),
        b"\xd8\x05" + b"\x80" * 16],
    # 8-bit Ext
    ["8-bit ext", msgpack.ExtType(0x05, b"\x80" * 255),
        b"\xc7\xff\x05" + b"\x80" * 255],
    # 16-bit ExtTypeType
    ["16-bit ExtType", msgpack.ExtType(0x05, b"\x80" * 256),
        b"\xc8\x01\x00\x05" + b"\x80" * 256],
    # 32-bit ExtType
    ["32-bit ExtType", msgpack.ExtType(0x05, b"\x80" * 65536),
...
composite_test_vectors = [
    ...
    # Complex Array
    ["complex array", [True, 0x01, msgpack.ExtType(0x03, b"foo"), 0xff,
...
pack_exception_test_vectors = [
    ...
    ["unsupported type", set([1, 2, 3]), msgpack.UnsupportedTypeException],
    ["unsupported type", -2**(64 - 1) - 1, msgpack.UnsupportedTypeException],
    ["unsupported type", 2**64, msgpack.UnsupportedTypeException],
]
...
unpack_exception_test_vectors = [
    ...
    ["insufficient data 8-bit uint", b"\xcc",
        msgpack.InsufficientDataException],
    ...
    ["insufficient data 32-bit map", b"\xdf\x00\x01\x00\x00\xc2\xc3",
        msgpack.InsufficientDataException],
    ["insufficient data fixExtType 1", b"\xd4",
        msgpack.InsufficientDataException],
    ["insufficient data fixExtType 1", b"\xd4\x05",
        msgpack.InsufficientDataException],
    ["insufficient data fixExtType 2", b"\xd5\x05\x01",
        msgpack.InsufficientDataException],
    ["insufficient data fixExtType 4", b"\xd6\x05\x01\x02\x03",
        msgpack.InsufficientDataException],
    ["insufficient data fixExtType 8", b"\xd7\x05\x01\x02\x03",
        msgpack.InsufficientDataException],
    ["insufficient data fixExtType 16", b"\xd8\x05\x01\x02\x03",
        msgpack.InsufficientDataException],
    ["insufficient data ExtType 8-bit", b"\xc7\x05\x05\x01\x02\x03",
        msgpack.InsufficientDataException],
    ["insufficient data ExtType 16-bit", b"\xc8\x01\x00\x05\x01\x02\x03",
        msgpack.InsufficientDataException],
    ["insufficient data ExtType 32-bit", b"\xc9\x00\x01\x00\x00\x05\x01\x02\x03",
        msgpack.InsufficientDataException],
    # Unhashable key { 1 : True, { 1 : 1 } : False }
    ["unhashable key", b"\x82\x01\xc3\x81\x01\x01\xc2",
        msgpack.UnhashableKeyException],
    ...
    ["duplicate key", b"\x82\x01\xc3\x01\xc2",
        msgpack.DuplicateKeyException],
    # Reserved code (0xc1)
    ["reserved code", b"\xc1",
        msgpack.ReservedCodeException],
    # Invalid string (non utf-8)
    ["invalid string", b"\xa1\x80",
        msgpack.InvalidStringException],
]
...
ext_handlers = {
    complex: lambda obj: msgpack.ExtType(0x20, struct.pack("ff", obj.real, obj.imag)),
    CustomType: lambda obj: msgpack.ExtType(0x30, msgpack.packb(list(obj))),
    0x20: lambda ext: complex(*struct.unpack("ff", ext.data)),
    0x30: lambda ext: CustomType(*msgpack.unpackb(ext.data)),
}
...
# These are the only global variables that should be exported by msgpack
...
class TestUmsgpack(unittest.TestCase):
    ...
    def test_pack_single(self):
        for (name, obj, data) in single_test_vectors:
            ...
            self.assertEqual(msgpack.packb(obj), data)
    """
    ...
    def test_unpack_composite(self):
        for (name, obj, data) in composite_test_vectors:
            ...
            self.assertEqual(msgpack.unpackb(data), obj)
    ...
    def test_unpack_invalid_string(self):
        ...
        obj = msgpack.unpackb(data, allow_invalid_utf8=True)
        self.assertTrue(isinstance(obj, msgpack.InvalidString))
    ...
    def test_unpack_ordered_dict(self):
        ...
        unpacked = msgpack.unpackb(data, use_ordered_dict=False)
        self.assertTrue(isinstance(unpacked, dict))

        # Unpack with ordered dict
        unpacked = msgpack.unpackb(data, use_ordered_dict=True)
        self.assertTrue(isinstance(unpacked, OrderedDict))
        self.assertEqual(unpacked, obj)

    def test_ext_exceptions(self):
        with self.assertRaises(TypeError):
            _ = msgpack.ExtType(-1, b"")

        with self.assertRaises(TypeError):
            _ = msgpack.ExtType(128, b"")

        with self.assertRaises(TypeError):
            _ = msgpack.ExtType(0, u"unicode string")
    ...
    def test_pack_ext_handler(self):
        for (name, obj, data) in ext_handlers_test_vectors:
            ...
            packed = msgpack.packb(obj, ext_handlers=ext_handlers)
    ...
    def test_unpack_ext_handler(self):
        for (name, obj, data) in ext_handlers_test_vectors:
            ...
            unpacked = msgpack.unpackb(data, ext_handlers=ext_handlers)
            self.assertEqual(unpacked, obj)
    ...
    def test_pack_force_float_precision(self):
        for ((name, obj, data), precision) in zip(float_precision_test_vectors, ["single", "double"]):
            ...
            packed = msgpack.packb(obj, force_float_precision=precision)
            self.assertEqual(packed, data)
    ...
    def test_streaming_reader(self):
        ...
        self.assertEqual(msgpack.unpack(reader), obj)

    def test_namespacing(self):
        # Get a list of global variables from msgpack module
